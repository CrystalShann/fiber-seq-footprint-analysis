filter for the overlap between accessible region and cCRE annotations
 - we want to do the filtering because cCRE annotations contain all cell types and mCpG fraction is higher than expected at enhancers and CTCF region
 - DNase-seq data /project/spott/cshan/annotations/DNase_ENCFF073ORT.bed
...
Find the overlap between CRE and DNase bed file
```{r find-overlap}
...
output_dir <- "/project/spott/cshan/fiber-seq/results/cCRE_summary/filtered_DNase_cCRE_mCpG"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

stopifnot(file.exists(dnase_path), file.exists(ccre_path))
...
filtered_ccre_dt <- ccre_dt[ccre_idx]

summary_dt <- filtered_ccre_dt[, .N, by = ccre_class][order(-N)]
summary_dt[, fraction := N / sum(N)]

data.table::fwrite(
  filtered_ccre_dt,
  file.path(output_dir, "DNase_overlapping_cCREs.bed"),
  sep = "\t",
  col.names = FALSE
)
data.table::fwrite(
  summary_dt,
  file.path(output_dir, "DNase_overlapping_cCREs_class_summary.tsv"),
  sep = "\t"
)
data.table::fwrite(
  filtered_ccre_dt,
  file.path(output_dir, "DNase_overlapping_cCREs.tsv.gz"),
  sep = "\t"
)

message(sprintf("Total cCREs: %d", nrow(ccre_dt)))
message(sprintf("DNase-overlapping cCREs: %d", nrow(filtered_ccre_dt)))
message(sprintf("Saved overlap BED + class summary in: %s", output_dir))
...
bin_size <- ifelse(length(args) >= 3, as.integer(args[[3]]), 1L)
if (is.na(bin_size) || bin_size < 1L) {
...
write_gz_tsv(
  n_ccre_class,
  file.path(ccre_output_dir, sprintf("%s_%s_filtered_DNase_n_ccre_class.tsv.gz", default_sample, default_chrom))
)
write_gz_tsv(
  meta_full,
  file.path(ccre_output_dir,
            sprintf("%s_%s_filtered_DNase_ccre_pbcpg_5mc_metaprofile.tsv.gz", default_sample, default_chrom))
)
...
```{r plot-from-saved-outputs}
args <- commandArgs(trailingOnly = TRUE)
default_sample <- ifelse(length(args) >= 1, args[[1]], "AL10_bc2178_19130")
default_chrom  <- ifelse(length(args) >= 2, args[[2]], "chr1")
plot_bin_size <- ifelse(length(args) >= 3, as.integer(args[[3]]), 1L)
ccre_output_dir <- "/project/spott/cshan/fiber-seq/results/cCRE_summary/filtered_DNase_cCRE_mCpG"
if (is.na(plot_bin_size) || plot_bin_size < 1L) {
  stop("plot_bin_size must be a positive integer", call. = FALSE)
}
...
ccre_5mc_path <- file.path(
  ccre_output_dir,
  sprintf("%s_%s_filtered_DNase_ccre_pbcpg_5mc.tsv.gz", default_sample, default_chrom)
)
meta_full_path <- file.path(
  ccre_output_dir,
  sprintf("%s_%s_filtered_DNase_ccre_pbcpg_5mc_metaprofile.tsv.gz", default_sample, default_chrom)
)
class_order_path <- file.path(
  ccre_output_dir,
  sprintf("%s_%s_filtered_DNase_class_order.tsv.gz", default_sample, default_chrom)
)
n_ccre_class_path <- file.path(
  ccre_output_dir,
  sprintf("%s_%s_filtered_DNase_n_ccre_class.tsv.gz", default_sample, default_chrom)
)

assert_file_exists(ccre_5mc_path, "ccre_5mc_all TSV")
assert_file_exists(meta_full_path, "meta_full TSV")
assert_file_exists(class_order_path, "class_order TSV")
assert_file_exists(n_ccre_class_path, "n_ccre_class TSV")

ccre_5mc_all <- data.table::fread(ccre_5mc_path)
meta_full <- data.table::fread(meta_full_path)
class_order_dt <- data.table::fread(class_order_path)
n_ccre_class <- data.table::fread(n_ccre_class_path)
class_order <- class_order_dt$region_class

ccre_5mc_all[, region_class := factor(region_class, levels = class_order)]
meta_full[, region_class := factor(region_class, levels = class_order)]

meta_full_plot <- data.table::copy(meta_full)
if (plot_bin_size > 1L) {
  meta_full_plot[, bin := as.integer(round(bin / plot_bin_size) * plot_bin_size)]
  meta_full_plot <- meta_full_plot[, .(
    modified_calls = sum(modified_calls, na.rm = TRUE),
    unmodified_calls = sum(unmodified_calls, na.rm = TRUE),
    total_calls = sum(total_calls, na.rm = TRUE),
    n_cpg_obs = sum(n_cpg_obs, na.rm = TRUE)
  ), by = .(region_class, bin)]
  meta_full_plot[, fraction_modified := data.table::fifelse(
    total_calls > 0, modified_calls / total_calls, NA_real_
  )]
}
...
all_ccre_5mc_plot <- ggplot2::ggplot(
  ...
  ggplot2::geom_jitter(width = 0.2, size = 0.6, alpha = 0.25, color = "#8C510A", na.rm = TRUE) +
  ggplot2::scale_x_discrete(labels = x_labels) +
  ggplot2::scale_y_continuous(expand = ggplot2::expansion(mult = 0.02)) +
  ggplot2::labs(
    title = sprintf("CpG-only 5mC fraction by cCRE class - DNase-overlapping %s cCREs (%s)",
...
meta_window_bp <- max(abs(meta_full_plot$bin), na.rm = TRUE)
meta_plot <- ggplot2::ggplot(
  meta_full_plot[!is.na(fraction_modified)],
  ...
  ggplot2::geom_smooth(
    ...
    color = "#8C510A", linewidth = 0.9, na.rm = TRUE
  ) +
  ...
  ggplot2::labs(
    title = sprintf(
      "CpG-only 5mC metaprofile around cCRE centers - DNase-overlapping %s cCREs (%s)",
      default_chrom, default_sample
    ),
...
ggplot2::ggsave(
  file.path(ccre_output_dir,
            sprintf("%s_%s_filtered_DNase_ccre_pbcpg_5mc.pdf", default_sample, default_chrom)),
  all_ccre_5mc_plot, width = 14, height = 6
)
ggplot2::ggsave(
  file.path(ccre_output_dir,
            sprintf("%s_%s_filtered_DNase_ccre_pbcpg_5mc_metaprofile.pdf", default_sample, default_chrom)),
