...
stop(
  paste(
    "run_modbam_promoter_summary_part2.R is deprecated.",
    "Run run_modbam_promoter_summary_m6A.R and run_modbam_promoter_summary_5mC.R",
    "as separate jobs, then run run_modbam_promoter_summary_join.R."
  ),
  call. = FALSE
)
...
# ---------------------------------------------------------------------------
source("/project/spott/cshan/fiber-seq/footprintR_modbam_code/modbam_footprintR_functions.R")
...
make_output_paths <- function(prefix, mod_label) {
  list(
    ...
    summary = file.path(output_dir, sprintf("%s_%s_%s.tsv.gz", default_sample, default_chrom, prefix)),
...
process_modality_chunked <- function(modbase_code, summary_prefix, mod_label, meta_label) {
  paths <- make_output_paths(summary_prefix, mod_label)
  all_paths <- unlist(paths, use.names = FALSE)

  if (all(file.exists(all_paths))) {
    message(mod_label, " outputs already exist; skipping.")
    return(data.table::fread(paths$summary))
  }

  # Recompute this modality from scratch if the output set is incomplete.
  ...
  if (!n_tss) {
    stop("No TSS rows found in part1 tss table.", call. = FALSE)
  }
  ...
  bin_dt <- data.table::data.table(bin = seq(-window_bp, window_bp, by = bin_size))
  meta_acc <- copy(bin_dt)
...
final_outputs <- c(
  file.path(output_dir, sprintf("%s_%s_m6A_position_calls.tsv.gz", default_sample, default_chrom)),
  file.path(output_dir, sprintf("%s_%s_m6A_modified_not_modified.tsv.gz", default_sample, default_chrom)),
  file.path(output_dir, sprintf("%s_%s_m6A_tss_aligned_positions.tsv.gz", default_sample, default_chrom)),
  file.path(output_dir, sprintf("%s_%s_m6A_tss_aligned_modified_not_modified.tsv.gz", default_sample, default_chrom)),
  ...
  file.path(output_dir, sprintf("%s_%s_5mC_position_calls.tsv.gz", default_sample, default_chrom)),
  file.path(output_dir, sprintf("%s_%s_5mC_modified_not_modified.tsv.gz", default_sample, default_chrom)),
  file.path(output_dir, sprintf("%s_%s_5mC_tss_aligned_positions.tsv.gz", default_sample, default_chrom)),
  file.path(output_dir, sprintf("%s_%s_5mC_tss_aligned_modified_not_modified.tsv.gz", default_sample, default_chrom)),
...
if (all(file.exists(final_outputs))) {
  message("Part 2 outputs already exist; skipping modification and summary stage.")
  m6a_summary <- data.table::fread(file.path(output_dir, sprintf("%s_%s_promoter_accessibility.tsv.gz", default_sample, default_chrom)))
  m5c_summary <- data.table::fread(file.path(output_dir, sprintf("%s_%s_promoter_methylation.tsv.gz", default_sample, default_chrom)))
} else {
  m6a_summary <- process_modality_chunked(
    modbase_code = "a",
  ...
  m5c_summary <- process_modality_chunked(
    modbase_code = "m",
...
# ---------------------------------------------------------------------------
# Promoter multi-omic summary (structure + accessibility + methylation)
# ---------------------------------------------------------------------------
message("Joining promoter multi-omic summary ...")
...
message("Multi-omic summary written.")

rm(m6a_summary, m5c_summary, promoter_multiomic_summary)
gc()
